...
describe('Templates command', () => {
  ...
  after(async () => { await deleteTemplateData() });
  afterEach(() => { fs.removeSync(dataFolder) });

  describe('Pull', () => {
    function retrieveFiles(path: string, excludeLayouts?: boolean) {
      const folderTree = dirTree(path, excludeLayouts && {exclude: /_layouts$/});
    ...
    describe('Templates', () => {
      it('templates', async () => {
        ...
        const folderTree = dirTree(dataFolder, {exclude: /_layouts$/});
      ...
      it('single template - file names', async () => {
        ...
        const names: string[] = files.map((f: any) => {return f.name});
      ...
      it('single template files - none empty', async () => {
        ...
        const result = files.findIndex((f: any) => {return f.size <= 0});
