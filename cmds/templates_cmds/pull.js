...
exports.handler = (argv) => {
  ...
  // Fetch template List
  sourceServer.getTemplates().then(response => {
    processTemplates({
      sourceServer,
...
const processTemplates = (config) => {
  ...
  templates.forEach(template => {
    sourceServer.getTemplate(template.TemplateId).then(response => {
      requestCount++;
      ...
      if (requestCount === totalCount) {
        ...
        spinner.stop(true);
        console.log(chalk.green(`All done! ${totalCount} have been saved to ${outputDir}.`))
      }
    }).catch(error => {
      spinner.stop(true);
      console.error(chalk.red(JSON.stringify(error)));
